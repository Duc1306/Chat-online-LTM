...
{
    if (group_name == NULL || creator == NULL)
        return -1;
...
{
    if (group_name == NULL || creator == NULL || members_list == NULL)
        return -1;
...
void handle_friend_remove(Message *msg)
{
    ...
    if (!fp || !temp)
    {
        ...
        if (from_socket >= 0) send_message_struct(from_socket, &error_msg);
        return;
    }
    ...
    mutex_unlock(&server_state.file_mutex); // ✅ UNLOCK TRƯỚC KHI SEND
    ...
    // ✅ giờ mới gửi friend list (không còn deadlock)
    if (from_socket >= 0) send_friends_list(from_socket, from_user);
    if (to_socket   >= 0) send_friends_list(to_socket, to_user);
    ...
    if (to_socket >= 0)
    {
        Message notify_msg;
        char notify_content[MAX_MESSAGE_LEN];
        snprintf(notify_content, sizeof(notify_content),
                 "%s removed you from their friend list", from_user);
    ...
    {
        Message ok;
        create_response_message(&ok, MSG_RESPONSE_SUCCESS, "SERVER", from_user, "Friend removed");
        send_message_struct(from_socket, &ok);
    }
